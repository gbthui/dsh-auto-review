...
export function parseVerdict(text: string): Verdict {
  ...
  const parsed = JSON.parse(body.slice(start, end + 1)) as { decision?: unknown; risk?: unknown; reason?: unknown }
...
export async function callReviewer(
  ...
  if (!key) throw new Error('no reviewer API key available (check apiKey / apiKeyEnv / apiKeyFile)')
