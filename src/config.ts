...
export const Config = z.object({
  ...
  reviewer: z.object({
     ...
     * authorization. 'off' disables reasoning on the request side
     * (endpoint path sends DeepSeek's `thinking:{type:disabled}`; session
     * path maps to `reasoningEffort: off` on `deepseek-official`).
     */
    thinking: z.union(['default', 'off']).default('default'),
    extraSystemPrompt: z.string().default(''),
    /**
     * Bounded read-only fact finding. The reviewer may request local path,
     * directory, and git observations before its final verdict. Fixed tools
     * only: no shell, network, escalation, or nested approval request. File
     * content is available only through inspect_text_file when
     * factFinding.content.enabled is true.
     */
    factFinding: z.object({
      ...
      /**
       * File-content inspection (inspect_text_file). Off by default. When an
       * external reviewer.baseURL is configured, enabling this explicitly
       * allows bounded workspace content to be sent there. Workspace-only,
       * binary-denied, sensitive-path-denied, and capped by maxBytes.
       */
      content: z.object({
  ...
  policy: z.object({
     ...
     * (`allowed-once`) without review. The shape is deliberately narrow:
     * exact tool name, literal operation prefixes, and an escalation target
     * that cannot exceed `workspace-write`. Regular expressions are not
     * supported because a false positive here is a permission incident.
     */
    ...
    /**
     * Structured evidence for the reviewer: recent user messages (trusted
     ...
     * plugin-injected context are never included. Older context may be
     * trimmed; the latest user authorization is never truncated. If that
     * latest message exceeds maxChars, the approval request fails closed.
     */
    context: z.object({
      enabled: z.boolean().default(true),
       ...
       * facts (output length + sha256), never raw text. true also includes
       * capped raw result text.
       */
