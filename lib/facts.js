 ...
 */
export function isSensitiveContentPath(realPath) {
    const normalized = String(realPath).replace(/\\/g, '/');
    if (CONTENT_DENY_PATH_PATTERNS.some((pattern) => pattern.test(normalized)))
        return true;
    const parts = normalized.split('/');
    const base = parts[parts.length - 1] ?? '';
    if (parts.some((part) => /^\.env(\..*)?$/i.test(part)))
        return true;
    if (parts.some((part) => /^\.(ssh|aws|azure|config|kube|gnupg|docker|npm)$/i.test(part)))
        return true;
    if (parts.some((part) => /^\.(npmrc|pypirc)$/i.test(part)))
        return true;
    if (/^\.(git-credentials|netrc)$/i.test(base))
        return true;
    if (/^(id_rsa|id_ed25519|id_dsa|id_ecdsa)(\..*)?$/i.test(base))
        return true;
    if (/\.(pem|key|p12|pfx)$/i.test(base))
        return true;
    if (/^(credentials?|secrets?)\.(json|toml|yaml|yml|ini|env)$/i.test(base))
        return true;
    return false;
}
 ...
 */
const execGit = (args, cwd, hooksDir, timeoutMs = 5000) => execFileAsync('git', ['-c', 'core.fsmonitor=false', '-c', 'core.hooksPath=' + hooksDir, ...args], { cwd, timeout: timeoutMs, maxBuffer: 1024 * 1024, windowsHide: true })
    .then((result) => String(result.stdout));
/** Strip credentials from a git remote URL and reduce it to structured fields. */
export function sanitizeGitUrl(raw) {
 ...
 * filesystem.
 */
async function containedRealPath(workspaceRoot, queryPath) {
...
export async function executeFacts(queries, workspaceRoot, options) {
    const observations = [];
                ...
                if (query.tool === 'inspect_path' || query.tool === 'inspect_directory' || query.tool === 'inspect_file_metadata') {
                    ...
                    if (st.isDirectory()) {
                        ...
                        for (const entry of entries.slice(0, 200)) {
                            try {
                                if ((await stat(path.join(real, entry))).isDirectory())
                                    dirs++;
                            }
                            catch {
                    ...
                    try {
                        ...
                        observations.push({ tool: query.tool, params, result: { remote: query.remote, error: String(error?.message ?? error).slice(0, 120) } });
                    }
                }
                    ...
                    try {
                        ...
                        observations.push({ tool: query.tool, params, result: { error: String(error?.message ?? error).slice(0, 120) } });
                    }
                }
            }
            catch (error) {
                observations.push({ tool: query.tool, params, result: { exists: false, error: String(error?.message ?? error).slice(0, 120) } });
            }
        }
    }
    finally {
        if (hooksDir)
            await rm(hooksDir, { recursive: true, force: true }).catch(() => { });
    }
    return observations;
}
